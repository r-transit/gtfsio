...
The function also includes a `quiet` argument that may shed some light on a few problems that your feed might have. Reading `bad_gtfs.zip`, for example, results in a few `data.table::fread()` warnings that are not very informative. `quiet` message output is more verbose, which helps to diagnose the source of the warnings:

```{r}
bad_path <- file.path(data_dir, "bad_gtfs.zip")

gtfs <- import_gtfs(bad_path)

suppressWarnings(
  gtfs <- import_gtfs(bad_path, quiet = FALSE)
)
```
